...
export const FOUNDER = {
  eyebrow: "만드는 사람",
  title: "기획부터 운영까지, 제품 전 과정을 설계합니다",
  ...
  body: [
    "5년 이상의 웹·서버 개발 경력을 바탕으로 서비스의 기획, 설계, 개발, 배포, 운영을 수행해 왔습니다. 이 중 3년 5개월간 10개 이상의 프로젝트를 총괄하며 웹·하이브리드 앱, 인프라 구축, 로봇 통신 시스템 등 다양한 환경에서 요구사항을 실제 서비스로 구현했습니다.",
    "현재는 서버 개발자로서 백엔드와 클라우드 인프라를 중심으로 전문성을 쌓고 있습니다. 프론트엔드부터 서버까지 아우르는 개발 경험을 바탕으로, 사용자 경험과 시스템 구조를 함께 고려하고 안정적인 운영과 유지보수를 위한 기반을 설계합니다.",
    "이렇게 쌓아 온 경험에 AI와의 협업이 새로운 가능성을 더했습니다. 디자인 시스템 Aurora Ledger와 약관·정책 플랫폼 Polaris를 만들고, 그 기반 위에서 물타기 계산기를 완성했습니다. 서로 다른 분야에서 쌓은 경험이 하나의 제품으로 이어진 결과입니다. 앞으로도 일상에서 느끼는 작은 불편에 귀 기울이며, 직접 만든 도구로 여러분과 만나고 싶습니다.",
  ],
} as const;
